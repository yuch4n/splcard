$(window).on('load', function() {
    $.ajax({
        ...
        success: function(data) {
            // Splatoon1
            data.udemae.forEach(function(ude) {
    ...
    $('#make2').on('click', function() {
        ...
        var udemae_area_Splus_gauge = $('#splatoon2 #udemae_area_Splus_gauge').html();
        var udemae_yagura_Splus_gauge = $('#splatoon2 #udemae_yagura_Splus_gauge').html();
        var udemae_hoko_Splus_gauge = $('#splatoon2 #udemae_hoko_Splus_gauge').html();
        var udemae_asari_Splus_gauge = $('#splatoon2 #udemae_asari_Splus_gauge').html();
    ...
    $('#splatoon2 select[id^=udemae_]').on('change', function() {
        if($(this)[0].value == 'S+') {
            ...
            $('#Splusgauge-dialog #gaugeval').html('0');
    ...
    $('#splatoon2 span.badge').on('click', function() {
        ...
        var sli = $('#Splusgauge-dialog input').slider();
        sli.slider('setValue', $(this).html());
        $('#Splusgauge-dialog #gaugeval').html($(this).html());
        $('#Splusgauge-dialog').modal('show');
    });
    ...
    $('#Splusgauge-dialog button.btn-info').on('click', function() {
        ...
        $('#splatoon2 span#'+name+'_Splus_gauge').html(value);
    });
    ...
    $('#Splusgauge-dialog input').on('change', function() {
        $('#Splusgauge-dialog #gaugeval').html($(this).slider('getValue'));
    });
    ...
    $('#buki_inputmethod').on('click', function() {
        if($('#buki').prop('tagName')=='SELECT') {
          $('#buki').replaceWith('<input type="text" class="form-control" id="buki" value="" />');
          $('#buki_inputmethod').children('span').removeClass('glyphicon-pencil');
          $('#buki_inputmethod').children('span').addClass('glyphicon-list');
        } else {
          $('#buki').replaceWith('<select class="form-control" id="buki"></select>');
          $('#buki_inputmethod').children('span').removeClass('glyphicon-list');
          $('#buki_inputmethod').children('span').addClass('glyphicon-pencil');
          $.ajax({
              type: "GET",
              url: "/js/splatoon_data.json",
              dataType: "json",
    ...
    if(('localStorage' in window) && (window.localStorage !== null)) {
        ...
        if(formData) {
            if('splatoon2' in formData) {
                ...
                form.udemae_area_Splus_gauge ? $('#splatoon2 #udemae_area_Splus_gauge').html(form.udemae_area_Splus_gauge):0;
                form.udemae_yagura ? $('#splatoon2 #udemae_yagura').val(form.udemae_yagura):0;
                form.udemae_yagura_Splus_gauge ? $('#splatoon2 #udemae_yagura_Splus_gauge').html(form.udemae_yagura_Splus_gauge):0;
                form.udemae_hoko ? $('#splatoon2 #udemae_hoko').val(form.udemae_hoko):0;
                form.udemae_hoko_Splus_gauge ? $('#splatoon2 #udemae_hoko_Splus_gauge').html(form.udemae_hoko_Splus_gauge):0;
                form.udemae_asari ? $('#splatoon2 #udemae_asari').val(form.udemae_asari):0;
                form.udemae_asari_Splus_gauge ? $('#splatoon2 #udemae_asari_Splus_gauge').html(form.udemae_asari_Splus_gauge):0;
...
function saveFormData() {
    if(('localStorage' in window) && (window.localStorage !== null)) {
        ...
        formData.splatoon2.udemae_area_Splus_gauge = $('#splatoon2 #udemae_area_Splus_gauge').html();
        formData.splatoon2.udemae_yagura = $('#splatoon2 #udemae_yagura').val();
        formData.splatoon2.udemae_yagura_Splus_gauge = $('#splatoon2 #udemae_yagura_Splus_gauge').html();
        formData.splatoon2.udemae_hoko = $('#splatoon2 #udemae_hoko').val();
        formData.splatoon2.udemae_hoko_Splus_gauge = $('#splatoon2 #udemae_hoko_Splus_gauge').html();
        formData.splatoon2.udemae_asari = $('#splatoon2 #udemae_asari').val();
        formData.splatoon2.udemae_asari_Splus_gauge = $('#splatoon2 #udemae_asari_Splus_gauge').html();
        ...
        formData.splatoon2.buki_inputmethod = $('#buki').prop('tagName');
